...
- **Live Cluster Topology & Node Specifications**: Real-time display of Node 1 (Primary Master, PowerDNS 4.9, MariaDB) and Node 2 (Secondary Replica, PowerDNS 4.7, SQLite3) with one-click copyable IPv4/IPv6 addresses and independent /24 subnets (`77.90.60.0/24` & `94.249.188.0/24`) on AS215365.
...
- **Architectural & Deployment Showcase**: Highlights multi-subnet IP diversity, AXFR supermaster replication, native DNSSEC, Caddy on-demand TLS validation, and quickstart commands for Docker Compose and zero-touch VPS installation.
